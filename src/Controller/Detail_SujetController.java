...
public class Detail_SujetController implements Initializable {
    ...
    int j = 0;
    @FXML
    private AnchorPane mydescription;

    @Override
    public void initialize(URL url, ResourceBundle rb) {

        try {
            mytitre.setText(AffichageForumController.getSujet_à_ouvrir().getTitre());
            mydesc.setText(AffichageForumController.getSujet_à_ouvrir().getContenu());
            nom_user.setText(sujetService.FindNomUserByIdUser_events(AffichageForumController.getSujet_à_ouvrir().getId_User()));
            //   id_nbre_jaime.setText(String.valueOf(AffichageForumController.getSujet_à_ouvrir().getNbre_jaime()));
            id_nbre_jaime.setText(String.valueOf(sujetService.FindNombreIntersById_events(AffichageForumController.getSujet_à_ouvrir().getId_Event())));
            ...
            if (jaimeService.FindValeurJaimeByIdUserAndIdSujet(MainFX.user.getId_User(), AffichageForumController.getSujet_à_ouvrir().getId_Event()) ==0)
            {
                 id_jaime.setVisible(true);
                 id_jaimepas.setVisible(false);
            }
            else if (jaimeService.FindValeurJaimeByIdUserAndIdSujet(MainFX.user.getId_User(), AffichageForumController.getSujet_à_ouvrir().getId_Event()) ==1) {
                id_jaime.setVisible(true);
                 id_jaimepas.setVisible(false);
            }else {
                id_jaime.setVisible(false);
    ...
    private void afficher_liste_commentaires() throws SQLException {
        ...
        List<Comment> ff = commentaireService.readAll(AffichageForumController.getSujet_à_ouvrir().getId_Event());
        ff.forEach(e -> {
...
//textField[i].getStyleClass().add("recumsg");
    ...
    void ajoutComment(ActionEvent event) throws SQLException {
        Comment commentaire = new Comment(AffichageForumController.getSujet_à_ouvrir().getId_Event(), MainFX.user.getId_User(), text_comment.getText());
...
id_send.setVisible(false);
 ...
 sujetService.compterintersted(AffichageForumController.getSujet_à_ouvrir());
        
        if (jaimeService.FindValeurJaimeByIdUserAndIdSujet(MainFX.user.getId_User(), AffichageForumController.getSujet_à_ouvrir().getId_Event()) == 0
               ){
                jaimeService.ajouterJaime(new participer(MainFX.user.getId_User(), AffichageForumController.getSujet_à_ouvrir().getId_Event(), 2));
        }
        else if(jaimeService.FindValeurJaimeByIdUserAndIdSujet(MainFX.user.getId_User(), AffichageForumController.getSujet_à_ouvrir().getId_Event()) ==1) {
               jaimeService.incrementerjaime(MainFX.user.getId_User(),  AffichageForumController.getSujet_à_ouvrir().getId_Event());
                }
        id_nbre_jaime.setText(String.valueOf(sujetService.FindNombreIntersById_events(AffichageForumController.getSujet_à_ouvrir().getId_Event())));

        System.out.println(AffichageForumController.getSujet_à_ouvrir().getInterstednumber());
        id_jaime.setVisible(false);
        id_jaimepas.setVisible(true);
    }
  ...
  jaimeService.decrementerjaime(MainFX.user.getId_User(),  AffichageForumController.getSujet_à_ouvrir().getId_Event());
