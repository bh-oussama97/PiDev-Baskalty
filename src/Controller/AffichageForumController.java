...
public class AffichageForumController implements Initializable {
  ...
  @FXML
   ...
   private static EvenementService sujetService = new EvenementService();
     ...
     */
   private static events sujet_à_ouvrir = new events();
   
   public static events getSujet_à_ouvrir() {
        return sujet_à_ouvrir;
    }

    public static void setSujet_à_ouvrir(events sujet_à_ouvrir) {
   ...
   private void loadUserGrid() throws SQLException {
		List<events> sujets = fetchForums();
		...
		if (nodes.length > 0) {
			pnItems.setStyle("-fx-background-color : #a6a6a6");
			pnItems.toFront();
		}
	...
	private List<events> fetchForums() throws SQLException {
		try {
	...
	private void displaySujetDetails(Node node, events sujet) {
		Label item_titre = (Label) node.lookup(".item_titre");
		item_titre.setText(sujet.getTitre());
		Label item_description = (Label) node.lookup(".item_description");
		item_description.setText(sujet.getContenu());
                Label item_categorie = (Label) node.lookup(".item_categorie");
		item_categorie.setText(sujet.getLocation());
                Label item_date = (Label) node.lookup(".item_date");
		item_date.setText(sujet.getStart().toString());
		// other properties
		// ...
	}

	private void setupActions(Node node, events sujet, int index) {
	...
	private EventHandler<MouseEvent> OpenEventHandler(events sujet, int index) {
		return event -> {
			
                   try {
                       sujetService.compterintersted(sujet);
    ...
    private List<events> fetchNbreVues() throws SQLException {
		try {
	...
	private void loadNbreVues() throws SQLException {
		List<events> sujets = fetchNbreVues();
		Node[] nodes = new Node[sujets.size()];
