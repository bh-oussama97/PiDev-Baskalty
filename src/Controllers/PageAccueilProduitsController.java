...
public class PageAccueilProduitsController implements Initializable {
    ...
    @FXML
    private AnchorPane baskaltyanchorpane;
    ...
    private void contactus(ActionEvent event) throws IOException {
         ...
         AnchorPane contactus = FXMLLoader.load(getClass().getResource("/com/pidev/views/Contact.fxml"));
           Node node = (Node) contactus;
           middle.getChildren().clear();
           middle.getChildren().add((Node) contactus);
           
           FadeTransition ft = new FadeTransition(Duration.millis(1500));
           ft.setNode(contactus);
        ft.setFromValue(0.1);
        ft.setToValue(1);
        ft.setCycleCount(1);
        ft.setAutoReverse(false);
